...
function loadSecret(name) {
  const path = `${homedir()}/.config/stellar/identity/${name}.toml`;
  const toml = readFileSync(path, "utf8");
  const m = toml.match(/seed_phrase\s*=\s*"([^"]+)"/);
  if (m) {
    // seed phrase form — convert via Keypair
    // Stellar CLI uses SLIP-0010 path m/44'/148'/0' by default
    const { default: bip39 } = await import("bip39");
    const seed = await bip39.mnemonicToSeed(m[1]);
    // ed25519 derivation for Stellar
    const { derivePath } = await import("ed25519-hd-key");
    const { key } = derivePath("m/44'/148'/0'", seed.toString("hex"));
    return Keypair.fromRawEd25519Seed(key);
  }
  const sk = toml.match(/secret_key\s*=\s*"(S[A-Z0-9]+)"/);
  if (sk) return Keypair.fromSecret(sk[1]);
  throw new Error(`no secret found in ${path}`);
}
...
async function main() {
  const deployer = await loadSecret("vineland-mainnet-deployer");
  const merchant = await loadSecret("vineland-mainnet-merchant");
